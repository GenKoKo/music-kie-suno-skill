...
- Onboarding walkthrough feedback:
  ...
  - V6 migration (operator-verified docs.kie.ai spec): jobs API — `POST /api/v1/jobs/createTask` with nested `{model: "ai-music-api/generate", callBackUrl, input}` snake_case payload, polling via `GET /api/v1/jobs/recordInfo`, `data.task_id` taskId parsing, completion via `data.state`, tracks read from the stringified `data.resultJson` payload (verified against a live request); plan model pinned to the V6 family — default **V6_WILD** (more variation under the same style; `V6` / `V6_MINI` accepted on request); V3_5–V5_5 discontinued upstream; duration validated 10–360
  - V6 timing: per-task poll cap raised 10 → 15 min (V6 360 s tracks observed 171–215 s live, slower than V5_5's 91 s median and possibly queue-dependent); expectation copy updated in SKILL.md + both READMEs, plus a per-family behavior table (MINI fastest/lightest encode, WILD most variation, MINI track delivery can stagger across polls)
